...
def train(model, train_loader, criterion, optimizer , epochs , validation_loader , hook):
    '''
    TODO: Complete this function that can take a model and
          data loaders for training and will get train the model
          Remember to include any debugging/profiling hooks that you might need
    '''
    ## to be edited : add the profiler hooks ya m3lm
    
    print(f'the length of train_loader is (i think number of batches) {len(train_loader)} and length of train_loader.dataset is {len(train_loader.dataset)}')
    hook.set_mode(smd.modes.TRAIN)
    for epoch in range(epochs):
      ...
      for data,target in train_loader:
        ...
        running_loss +=loss
        predicted = predicted.max(1,keepdim = True)[1]
        true_labels += predicted.eq(target.view_as(predicted)).sum().item()
        loss.backward()
        optimizer.step()
      print(f'the running loss in the epoch {epoch} is {running_loss} and accuracy is {100*true_labels/len(train_loader.dataset)}')
